...
- For image processing, use Python libraries or another Gemini model that is working (no Gemini Vision).  
"""

    chat = await get_chat_session(parse_chat_sessions, session_id, SYSTEM_PROMPT)

    prompt = retry_message if retry_message else question_text
    ...
    # Send prompt through key rotation
